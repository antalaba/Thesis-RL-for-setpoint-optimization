# Updated version: removed the future-violation auxiliary network/losses and delayed latent matching losses until backbone_dynamics_enable_after_batch.
import os
import csv
import math
import random
...
from utils_added_weighted_signal import SupermarketFMUEnv
...
@dataclass
class Config:
    fmu_path: str = "A_Supermarket_CO2_Model_Improved_24_ode1_001.fmu"
    excel_path: str = "Price_Temp_FullYear_2022.xlsx"
    save_dir: str = "hybrid_reward_seq_cost_mlp_runs"
    run_name: str = "actor_window_lstm_transformer_switching_dynamics_publicpred_refrigeration"
    ...
    num_envs: int = 60
    sim_dt: float = 120.0 
    ctrl_dt: float = 120.0 * 5
    horizon: int =  int((24 * 60 * 60) / ctrl_dt) * 2

    episode_hours: int = 24
    forecast_hours: int = 8

    Psuc_ref_bounds: tuple = (18.0, 35.0)
    Troom_ref_bounds_C: tuple = (2.0, 8.0)
    Psuc_ref_init: float = 28.0
    Troom_ref_init_C: float = 3.75

    price_reward_weight: float = 5.0
    alive_reward_per_ctrl_step: float = 0.005
    action_smoothness_coef: float = 0.0001
    action_smoothness_type: str = "l2"
    action_smoothness_use_scaled: bool = True
    action_lpf_alpha: float = 1.0

    T_wmean_min_C: float = 3.5
    T_wmean_max_C: float = 4.0
    ...
    fan_power_coeff: float = 0.02
    obs_divisors: dict | None = None

    actor_num_cells: int = 256
    actor_lstm_hidden: int = 256
    actor_window_size: int = 6
    actor_xfm_nhead: int = 4
    actor_xfm_layers: int = 2
    actor_xfm_ff_dim: int = 512
    actor_xfm_dropout: float = 0.0

    dyn_state_dim: int = 32
    dyn_num_regimes: int = 4
    dyn_hidden: int = 128
    dyn_loss_coef: float = 0.0001
    dyn_roll_horizon: int = 10
    dyn_rollout_ramp_batches: int = 40
    dyn_metric_min_scale: float = 0.10
    dyn_metric_huber_delta: float = 1.0

    latent_dyn1_coef: float = 0.10
    latent_roll_coef: float = 0.05

    temperature_signal_info_key: str = "T_weighted_C"
    current_temperature_obs_index: int | None = None
    temperature_residual_output_scale_C: float = 1.0
    temp_decoder_hidden: int = 128

    temp_residual_dyn1_coef: float = 6.0
    temp_residual_roll_coef: float = 14.0
    # Keep the same auxiliary temperature-supervision branches as the Sinergym HVAC script.
    # These make re-anchored rollouts and rollout-bias terms part of the actual gradient path.
    temp_reanchor_roll_coef: float = 6.0
    temp_roll_bias_coef: float = 3.0

    viol_dyn1_coef: float = 10.0
    viol_roll_coef: float = 12.0
    viol_tail_coef: float = 3.0

    temp_persist_dyn1_coef: float = 12.0
    temp_persist_roll_coef: float = 24.0
    viol_persist_dyn1_coef: float = 8.0
    viol_persist_roll_coef: float = 16.0

    temp_gain_tail_coef: float = 6.0
    temp_gain_consistency_coef: float = 3.0
    temp_persist_margin: float = 0.0
    temp_residual_scale: float = 0.50
    violation_loss_scale: float = 0.25
    train_reanchor_prob: float = 0.5
    train_reanchor_gap: int = 3
    temp_supervision_enable_after_batch: int = 0
    backbone_dynamics_enable_after_batch: int = 0

    dynamics_exog_obs_indices: tuple | None = (3,)
    forecast_ambient_obs_indices: tuple | None = None
    use_obs_in_state_projector: bool = True
    state_projector_obs_clip: float = 10.0


    shield_enable: bool = False
    shield_apply_after_batch: int = 15
    shield_horizon: int = 30
    shield_opt_steps: int = 4
    shield_opt_lr: float = 0.25
    shield_opt_tol: float = 1e-6
    shield_opt_grad_clip: float = 1.0
    shield_opt_freeze_model: bool = True
    shield_detach_action_for_logprob: bool = True
    shield_violation_coef: float = 10.0
    shield_violation_max_coef: float = 5.0
    shield_deviation_coef: float = 0.25
    shield_trigger_violation: float = 0.0
    shield_only_if_triggered: bool = True
    shield_commit_max_steps: int = 1
    shield_frontload_decay: float = 0.65
    shield_frontload_min_weight: float = 0.25
    shield_early_horizon: int = 4
    shield_early_violation_coef: float = 2.0
    shield_early_violation_max_coef: float = 2.0

    total_frames: int = 10_000_000
    frames_per_batch: int = horizon * num_envs * 2
    chunk_T: int = horizon * 2
    ppo_epochs: int = 10
    
    actor_lr: float = 5e-5
    reward_critic_lr: float = 5e-5
    cost_critic_lr: float = 5e-5
    backbone_dyn_lr: float = 5e-5
    projector_lr: float = 5e-5
    dynamics_lr: float = 5e-5

    cuda_empty_cache_every_minibatches: int = 16
    empty_cache_after_checkpoint: bool = True

    reward_gamma: float = 0.99
    reward_lmbda: float = 0.98
    cost_gamma: float = 0.99
    cost_lmbda: float = 0.98

    clip_epsilon: float = 0.2
    entropy_coef: float = 1e-4
    reward_value_coef: float = 1.0
    ...
    cost_budget: float = 0.04
    lambda_warmup_batches: int = 0

    use_blended_training_cost: bool = False
    blended_cost_alpha_inst: float = 0.25
    blended_cost_beta_full: float = 1.0
    blended_cost_gamma_weighted: float = 1.0
...
def temperature_supervision_enabled_for_batch(batch_idx: int, cfg: Config) -> bool:
    return int(batch_idx) >= max(0, int(getattr(cfg, "temp_supervision_enable_after_batch", 0)))


def backbone_dynamics_enabled_for_batch(batch_idx: int, cfg: Config) -> bool:
    return int(batch_idx) >= max(0, int(getattr(cfg, "backbone_dynamics_enable_after_batch", 0)))
...
def tensor_rmse(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    return torch.sqrt(torch.mean((x - y) ** 2) + 1e-12)


def tensor_corrcoef(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    x = x.float().reshape(-1)
    y = y.float().reshape(-1)
...
def channelwise_target_scale(target: torch.Tensor, min_scale: float = 1e-3) -> torch.Tensor:
    ...
    scale = target.detach().float().std(dim=dims, unbiased=False)
...
def normalized_rmse(pred: torch.Tensor, target: torch.Tensor, scale: torch.Tensor | float) -> torch.Tensor:
    scale_t = torch.as_tensor(scale, device=pred.device, dtype=pred.dtype)
    while scale_t.ndim < pred.ndim:
        scale_t = scale_t.unsqueeze(0)
    return torch.sqrt(torch.mean(((pred - target) / scale_t.clamp_min(1.0e-6)) ** 2) + 1e-12)
...
def normalized_huber_loss(pred: torch.Tensor, target: torch.Tensor, scale: torch.Tensor | float, delta: float = 1.0) -> torch.Tensor:
    ...
    while scale_t.ndim < pred.ndim:
        scale_t = scale_t.unsqueeze(0)
    z = (pred - target) / scale_t.clamp_min(1.0e-6)
    return F.huber_loss(z, torch.zeros_like(z), delta=float(delta), reduction="mean")
...
def _sanitize_index_list(idxs, obs_dim: int) -> list[int]:
    if idxs is None:
        return []
    out = []
    for idx in idxs:
...
def extract_dynamics_exog_tensor(obs: torch.Tensor, cfg: Config) -> torch.Tensor:
    if obs is None:
        raise RuntimeError('extract_dynamics_exog_tensor received None obs')
    obs_dim = int(obs.shape[-1])
    idxs = _sanitize_index_list(getattr(cfg, 'dynamics_exog_obs_indices', None), obs_dim)
    if len(idxs) == 0:
        return obs.new_zeros(obs.shape[:-1] + (0,))
    idx = torch.as_tensor(idxs, device=obs.device, dtype=torch.long)
    return obs.index_select(-1, idx).float()


def build_exog_rollout_sequence_from_current_obs(obs: torch.Tensor, cfg: Config, horizon: int) -> torch.Tensor:
    base_exog = extract_dynamics_exog_tensor(obs, cfg)
    H = int(max(horizon, 0))
    if H <= 0:
        return base_exog.unsqueeze(-2)[..., :0, :]
    if base_exog.shape[-1] == 0:
        return base_exog.unsqueeze(-2).expand(*base_exog.shape[:-1], H, 0)
    seq = base_exog.unsqueeze(-2).expand(*base_exog.shape[:-1], H, base_exog.shape[-1]).clone()
    ambient_forecast_idxs = _sanitize_index_list(getattr(cfg, 'forecast_ambient_obs_indices', None), int(obs.shape[-1]))
    if len(ambient_forecast_idxs) > 0:
        for h in range(min(H, len(ambient_forecast_idxs))):
            seq[..., h, 0] = obs[..., ambient_forecast_idxs[h]].float()
        if len(ambient_forecast_idxs) < H:
            last_idx = ambient_forecast_idxs[-1]
            for h in range(len(ambient_forecast_idxs), H):
                seq[..., h, 0] = obs[..., last_idx].float()
    return seq



def _infer_default_current_temperature_obs_index(obs_dim: int, cfg: Config) -> int:
    explicit = getattr(cfg, "current_temperature_obs_index", None)
    if explicit is not None:
        ...
        if 0 <= idx < obs_dim:
            return idx
        raise RuntimeError(f"current_temperature_obs_index={explicit} is invalid for obs_dim={obs_dim}")
    forecast_idxs = _sanitize_index_list(getattr(cfg, "forecast_ambient_obs_indices", None), obs_dim)
    if len(forecast_idxs) > 0:
        guess = forecast_idxs[0] - 1
        if 0 <= guess < obs_dim:
            return guess
    if bool(getattr(cfg, "include_weighted_temp_in_obs", False)) and int(getattr(cfg, "forecast_hours", 0)) > 0 and obs_dim > int(getattr(cfg, "forecast_hours", 0)):
        guess = obs_dim - int(getattr(cfg, "forecast_hours", 0)) - 1
        if 0 <= guess < obs_dim:
            return guess
    return max(0, obs_dim - 1)


def _resolve_mean_temp_bounds_like(x: torch.Tensor, cfg: Config):
    lo_t = torch.full_like(x, float(cfg.T_wmean_min_C))
    hi_t = torch.full_like(x, float(cfg.T_wmean_max_C))
    return lo_t, hi_t


def clip_temp_for_constraint(temp_c: torch.Tensor, cfg: Config) -> torch.Tensor:
    lo_t, hi_t = _resolve_mean_temp_bounds_like(temp_c, cfg)
    clip_c = getattr(cfg, "violation_clip_C", None)
    if clip_c is None:
        return temp_c
    clip_c = float(clip_c)
    if clip_c <= 0.0:
    ...
    lo_clip = lo_t - clip_c
    hi_clip = hi_t + clip_c
    return torch.clamp(temp_c, min=lo_clip, max=hi_clip)


def mean_temp_violation(mean_temp_c: torch.Tensor, cfg: Config) -> torch.Tensor:
    mean_temp_c = clip_temp_for_constraint(mean_temp_c, cfg)
    lo_t, hi_t = _resolve_mean_temp_bounds_like(mean_temp_c, cfg)
    below = F.relu(lo_t - mean_temp_c)
    above = F.relu(mean_temp_c - hi_t)
    return below + above


def clip_temp_slot_value(x: torch.Tensor, cfg: Config) -> torch.Tensor:
    lo = getattr(cfg, "temperature_clip_min_C", None)
    hi = getattr(cfg, "temperature_clip_max_C", None)
    y = x.float()
    if lo is not None:
        y = y.clamp_min(float(lo))
    if hi is not None:
        y = y.clamp_max(float(hi))
    return y


def get_current_temperature_from_obs(obs: torch.Tensor, cfg: Config) -> torch.Tensor:
    obs_dim = int(obs.shape[-1])
    idx = _infer_default_current_temperature_obs_index(obs_dim, cfg)
    return clip_temp_slot_value(obs[..., idx].float(), cfg)


def get_temperature_signal_tensor_from_info(sub_td, cfg: Config, time_dim: int) -> torch.Tensor:
    key = str(getattr(cfg, 'temperature_signal_info_key', 'T_weighted_C'))
    x = safe_key(sub_td, ('next', key))
    if x is None and key != 'T_weighted_C':
        x = safe_key(sub_td, ('next', 'T_weighted_C'))
    if x is None and key != 'T_wmean_C':
        x = safe_key(sub_td, ('next', 'T_wmean_C'))
    if x is None:
        raise RuntimeError(f'temperature signal key not found in next info: {key!r}')
    x = to_TB(x.float(), time_dim=time_dim).movedim(0, 1)
    if x.ndim == 3 and x.shape[-1] == 1:
        x = x.squeeze(-1)
    return clip_temp_slot_value(x, cfg)
...
def choose_logging_horizons(max_h: int) -> list[int]:
    max_h = int(max(1, max_h))
    candidates = [1, 2, 4, 6, 8, 12, 24, max_h]
    out = []
    seen = set()
    for h in candidates:
        h = min(max_h, int(h))
...
def format_horizon_triplets(losses: dict, denom: float, model_prefix: str, persist_prefix: str, gain_prefix: str, horizons: list[int], label: str) -> str:
    parts = []
    for h in horizons:
        model_key = f"{model_prefix}_h{h:02d}"
        persist_key = f"{persist_prefix}_h{h:02d}"
        gain_key = f"{gain_prefix}_h{h:02d}"
        if model_key not in losses or persist_key not in losses or gain_key not in losses:
            continue
        model_v = losses[model_key] / denom
        persist_v = losses[persist_key] / denom
        gain_v = losses[gain_key] / denom
        parts.append(f"h{h:02d}[m={model_v:.4f} p={persist_v:.4f} g={gain_v:+.4f}]")
    return f"{label}: " + " | ".join(parts) if parts else f"{label}: NA"


def append_batch_metrics_csv(csv_path: str | Path, row: dict):
...
def batch_constraint_stats(td):
    c = safe_key(td, ("next", "constraint"))
    c = torch.zeros_like(td["next", "reward"]).float() if c is None else c.float()
    require_finite(c, "constraint (batch stats)")
    flat = c.reshape(-1)
    return flat.mean(), flat.std(unbiased=False), flat.max(), (flat > 1e-8).float().mean()
...
def instantaneous_temp_violation_torch(tcab, lo, hi, clip_c):
    below = torch.clamp(lo - tcab, min=0.0)
    above = torch.clamp(tcab - hi, min=0.0)
    v = torch.maximum(below, above)
    positive_clip = clip_c > 0.0
    if positive_clip.any():
        v = torch.where(positive_clip, torch.minimum(v, clip_c), v)
    return v


def make_constraint_from_info(td, cfg: Config):
    weighted = safe_key(td, ("next", "T_weighted_violation_always"))
    hard = safe_key(td, ("next", "T_wmean_violation_always"))
    if cfg.use_blended_training_cost:
        tcab = td["next", "Tcabinet_C"].float()
        lo = td["next", "Twmean_min_C"].float()
        hi = td["next", "Twmean_max_C"].float()
        clip_c = safe_key(td, ("next", "Twmean_violation_clip_C"))
        clip_c = torch.full_like(tcab, -1.0) if clip_c is None else clip_c.float()
        v_inst = instantaneous_temp_violation_torch(tcab, lo, hi, clip_c)
        v_full = td["next", "T_wmean_violation_full"].float()
        weighted = td["next", "T_wmean_violation_always"].float() if weighted is None else weighted.float()
        c = cfg.blended_cost_alpha_inst * v_inst + cfg.blended_cost_beta_full * v_full + cfg.blended_cost_gamma_weighted * weighted
        require_finite(c, "blended constraint")
        return c.float()
    if weighted is not None:
        weighted = weighted.float()
        require_finite(weighted, "T_weighted_violation_always")
        return weighted
    if hard is not None:
        hard = hard.float()
        require_finite(hard, "T_wmean_violation_always")
        return hard
    z = torch.zeros_like(td["next", "reward"]).float()
    require_finite(z, "constraint zero fallback")
    return z
...
class ClampLocScale(nn.Module):
    def __init__(self, min_scale=1e-3, max_scale=10.0):
        super().__init__()
        self.min_scale = float(min_scale)
        self.max_scale = float(max_scale)

    def forward(self, loc, scale):
        return loc, torch.clamp(scale, self.min_scale, self.max_scale)
...
class RollingWindowBuffer(nn.Module):
    ...
    def _step_update(self, x_bh, window_bwh, is_init_b=None):
        ...
        next_window = torch.cat([window_bwh[:, 1:, :], x_bh.unsqueeze(1)], dim=1)
        return next_window
...
def make_feature_module(obs_key: str, out_key: str, hidden: int, device):
...
class CriticMLP(nn.Module):
    def __init__(self, obs_dim: int, hidden_dims=(256, 256, 256)):
        super().__init__()
...
class LatentStateProjector(nn.Module):
    def __init__(self, in_dim: int, state_dim: int, hidden: int = 128, obs_clip: float = 10.0):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, obs: torch.Tensor | None = None):
        x = x.float()
        if obs is not None:
            obs = torch.clamp(obs.float(), -self.obs_clip, self.obs_clip)
...
def decode_temperature_from_q(q: torch.Tensor, temp_decoder: LatentTemperatureDecoder, cfg: Config, temp_base: torch.Tensor | None = None, exog: torch.Tensor | None = None) -> torch.Tensor:
    ...
    residual = residual_raw * float(getattr(cfg, 'temperature_residual_output_scale_C', 1.0))
    ...
    temperature_hat = clip_temp_slot_value(temperature_hat, cfg)
    require_finite(temperature_hat, 'decoded temperature')
    return temperature_hat


def temperature_terms_from_q(q: torch.Tensor, cfg: Config, temp_decoder: LatentTemperatureDecoder, temp_base: torch.Tensor | None = None, exog: torch.Tensor | None = None):
    temperature_hat = decode_temperature_from_q(q, temp_decoder, cfg, temp_base=temp_base, exog=exog)
    violation_mag = mean_temp_violation(temperature_hat, cfg)
    hard_violation = (violation_mag > 0.0).float()
    return violation_mag, hard_violation, temperature_hat


def temperature_violation_from_q(q: torch.Tensor, cfg: Config, temp_decoder: LatentTemperatureDecoder, temp_base: torch.Tensor | None = None, exog: torch.Tensor | None = None) -> torch.Tensor:
    violation_mag, *_ = temperature_terms_from_q(q, cfg, temp_decoder, temp_base=temp_base, exog=exog)
    return violation_mag


def dreamed_temperature_violation_from_q_sequence(q_seq: torch.Tensor, cfg: Config, temp_decoder: LatentTemperatureDecoder, temp_base: torch.Tensor | None = None, exog_seq: torch.Tensor | None = None):
    if q_seq.ndim != 3:
        raise RuntimeError(f"dreamed_temperature_violation_from_q_sequence expected [N,H+1,D], got {tuple(q_seq.shape)}")
    if q_seq.shape[1] <= 1:
    ...
    if temp_base is None:
        ...
        if base.ndim == q_future.ndim - 1:
            ...
            temp_base_seq = base
    return temperature_terms_from_q(q_future, cfg, temp_decoder, temp_base=temp_base_seq, exog=exog_seq)
...
class DynamicsSummaryModule(nn.Module):
    ...
    def forward(self, q: torch.Tensor, obs: torch.Tensor | None = None):
        exog = None
        if obs is not None and self.cfg is not None:
            exog = extract_dynamics_exog_tensor(obs, self.cfg)
...
class ActorLocScaleHead(nn.Module):
    """Policy head that tolerates both 2-arg and 3-arg construction styles."""

    def __init__(self, in_dim: int, hidden_dim: int, act_dim: int | None = None):
        super().__init__()
        if act_dim is None:
            act_dim = int(hidden_dim)
            hidden_dim = max(64, int(in_dim))
...
def weighted_temp_from_q(q: torch.Tensor, temp_decoder: LatentTemperatureDecoder, cfg: Config, exog: torch.Tensor | None = None) -> torch.Tensor:
    return decode_mean_temp_from_q(q, temp_decoder, cfg, exog=exog)
...
class ShieldedLatentPolicyActor(nn.Module):
    def __init__(self, actor_trunk, actor_policy_core, dyn_model, temp_decoder, low: torch.Tensor, high: torch.Tensor, cfg: Config):
        ...
        active_now = bool(cfg.shield_enable) and int(getattr(cfg, "shield_apply_after_batch", 0)) <= 0
    ...
    def set_shield_apply_enabled(self, enabled: bool):
        enabled = bool(enabled) and bool(getattr(self.cfg, "shield_enable", True))
        self.shield_eval_enabled = enabled
        self.shield_apply_enabled = enabled

    def _dist(self, loc: torch.Tensor, scale: torch.Tensor):
        return TanhNormal(loc, torch.clamp(scale, 1e-3, 1.0), low=self.low, high=self.high)

    def _policy_mean_from_latent(self, q: torch.Tensor, obs: torch.Tensor | None = None, exog_override: torch.Tensor | None = None):
        exog = exog_override
        if exog is None and obs is not None:
            exog = extract_dynamics_exog_tensor(obs, self.cfg)
    ...
    def _build_nominal_action_sequence(self, q0: torch.Tensor, first_action: torch.Tensor, exog_seq: torch.Tensor | None = None, obs: torch.Tensor | None = None):
        q = q0
        a_cur = first_action
        seq = []
        ...
        for h in range(H):
            ...
            if h < H - 1:
                ...
                a_cur, _, _ = self._policy_mean_from_latent(q, obs if exog_seq is None else None, exog_override=next_exog)
    ...
    def _rollout_objectives_from_q_sequence(self, q_seq: torch.Tensor, temp_base: torch.Tensor, exog_seq: torch.Tensor | None = None):
        pred_cost_seq, *_ = dreamed_temperature_violation_from_q_sequence(q_seq, self.cfg, self.temp_decoder, temp_base=temp_base, exog_seq=exog_seq)
        if pred_cost_seq.shape[1] == 0:
    ...
    def _rollout_objectives_from_first_action(self, q0: torch.Tensor, first_action: torch.Tensor, temp_base: torch.Tensor, exog_seq: torch.Tensor | None = None, obs: torch.Tensor | None = None):
        q = q0
        a_cur = first_action
        q_seq = [q]
        H = int(self.cfg.shield_horizon)
    ...
    def _optimize_action_sequence(self, q0: torch.Tensor, base_action: torch.Tensor, temp_base: torch.Tensor, exog_seq: torch.Tensor | None = None, obs: torch.Tensor | None = None):
        steps = int(self.cfg.shield_opt_steps)
        q0_det = q0.detach()
        base_det = base_action.detach()

        with torch.no_grad():
            nominal_seq = self._build_nominal_action_sequence(q0_det, base_det, exog_seq=exog_seq, obs=obs)
        ...
        grad_clip = float(getattr(self.cfg, "shield_opt_grad_clip", 0.0))
        ...
        with torch.enable_grad():
            for _ in range(steps):
                ...
                grad_pre_tanh, = torch.autograd.grad(score.sum(), pre_tanh, retain_graph=False, create_graph=False, allow_unused=False)
    ...
    def plan_shield_sequence(self, q0: torch.Tensor, base_action: torch.Tensor, obs: torch.Tensor | None = None):
        qd = q0.detach()
        ...
        with torch.no_grad():
            exog_seq = None if obs is None else build_exog_rollout_sequence_from_current_obs(obs.float(), self.cfg, horizon)
            temp_base = qd.new_zeros(qd.shape[:-1]) if obs is None else get_current_temperature_from_obs(obs.float(), self.cfg)
            base_stats = self._rollout_objectives_from_first_action(qd, bd, temp_base=temp_base, exog_seq=exog_seq, obs=obs)
            base_viol = base_stats["max_violation"]
            base_score = self._shield_score(base_stats, deviation=None)
            nominal_seq = self._build_nominal_action_sequence(qd, bd, exog_seq=exog_seq, obs=obs)
        ...
        if trigger.any().item():
            ...
            trig_obs = None if obs is None else obs[trigger]
            result = self._optimize_action_sequence(qd[trigger], bd[trigger], temp_base=temp_base[trigger], exog_seq=trig_exog, obs=trig_obs)
        ...
        use_opt = trigger & improved if bool(self.cfg.shield_only_if_triggered) else improved
    ...
    def _shield(self, q0: torch.Tensor, base_action: torch.Tensor, obs: torch.Tensor | None = None):
        ...
        plan = self.plan_shield_sequence(q0, base_action, obs=obs)
    ...
    def forward(self, td):
        ...
        action, shield_logs = self._shield(td["actor_dyn_state"].float(), base_action, obs.float() if isinstance(obs, torch.Tensor) else None)
        action_for_logp = action.detach() if bool(getattr(self.cfg, "shield_detach_action_for_logprob", True)) else action
...
def shield_apply_enabled_for_batch(batch_idx: int, cfg: Config) -> bool:
    if not bool(cfg.shield_enable):
        return False
    return int(batch_idx) >= max(0, int(cfg.shield_apply_after_batch))
...
def discounted_prefix_sums(x_bt: torch.Tensor, horizon: int, gamma: float) -> torch.Tensor | None:
    B, T = x_bt.shape
    if T <= horizon:
        return None
    out = x_bt.new_zeros(B, T - horizon)
    for i in range(horizon):
        out = out + (gamma ** i) * x_bt[:, i:i + T - horizon]
    return out
...
def rollout_latent_for_horizon_reanchored(q: torch.Tensor, a: torch.Tensor, exog_next: torch.Tensor | None, dyn_model: SwitchingLatentDynamics, horizon: int, reanchor_gap: int):
    """Open-loop rollout with periodic teacher re-anchoring, matching the Sinergym HVAC training flow.

    The prediction is rolled forward for at most `reanchor_gap` steps, then the latent state is
    replaced by the observed/projected latent at that intermediate time. This keeps the auxiliary
    rollout loss from becoming only a compounding-error loss.
    """
    T = int(q.shape[1])
    if int(horizon) < 1 or int(horizon) >= T:
        raise ValueError(f"Invalid horizon={horizon} for sequence length T={T}")
...
def latent_dynamics_loss(sub_td, state_proj_model: LatentStateProjector, dyn_model: SwitchingLatentDynamics, temp_decoder: LatentTemperatureDecoder, cfg: Config, batch_idx: int):
    ...
    obs = move_time_to_dim1(sub_td["observation"].float(), time_dim)
    next_obs = move_time_to_dim1(sub_td["next", "observation"].float(), time_dim)
    exog_now = extract_dynamics_exog_tensor(obs, cfg)
    exog_next = extract_dynamics_exog_tensor(next_obs, cfg)

    proj_obs = obs if bool(getattr(cfg, "use_obs_in_state_projector", True)) else None
    ...
    rollout_progress = min(1.0, float(batch_idx + 1) / float(max(int(getattr(cfg, "dyn_rollout_ramp_batches", 0)), 1))) if int(getattr(cfg, "dyn_rollout_ramp_batches", 0)) > 0 else 1.0

    q = state_proj_model(x_lat if backbone_dyn_on else x_lat.detach(), proj_obs)
    q_decoder = state_proj_model(x_lat.detach(), proj_obs)
    ...
    def _zero_logs():
        logs = {
            ...
            "temp_supervision_enable_after_batch": float(getattr(cfg, "temp_supervision_enable_after_batch", 0)),
            "backbone_dynamics_enabled": float(backbone_dyn_on),
            "backbone_dynamics_enable_after_batch": float(getattr(cfg, "backbone_dynamics_enable_after_batch", 0)),
    ...
    temp_now = get_current_temperature_from_obs(obs, cfg)
    temp_next_seq = get_temperature_signal_tensor_from_info(sub_td, cfg, time_dim)
    ...
    rmse_temp_direct = tensor_rmse(temperature_decoded, temp_now)
    ...
    latent_cost_now = mean_temp_violation(temperature_decoded, cfg)
    latent_hard_now = (latent_cost_now > 0.0).float()
    ...
    viol_hat_dyn1 = mean_temp_violation(temp_hat_dyn1, cfg)
    viol_tgt_dyn1 = mean_temp_violation(temp_tgt_dyn1, cfg)
    viol_base_dyn1 = mean_temp_violation(temp_base_dyn1, cfg)
    ...
    # Match the Sinergym HVAC loss: persistence penalties are based on RMSE, not NRMSE.
    ...
    if H >= 2:
        for idx, h in enumerate(range(2, H + 1)):
            ...
            viol_hat_h = mean_temp_violation(temp_hat_h, cfg)
            viol_tgt_h = mean_temp_violation(temp_tgt_h, cfg)
            viol_base_h = mean_temp_violation(temp_base_h, cfg)
...
def save_checkpoint(*, run_dir: str, policy, reward_critic, cost_critic, dynamics_model=None, temperature_decoder=None, actor_optim=None, dynamics_optim=None, reward_critic_optim=None, cost_critic_optim=None, lag_lambda=None, frames=None, batch_idx=None, term_counts=None, running_ep_steps=None, cfg: Config | None = None):
...
def build_env(cfg: Config) -> SupermarketFMUEnv:
    obs_divisors = cfg.obs_divisors or {"Wcomp": 100_000.0, "Qevap": 100_000.0, "Tcabinet": 10.0, "T_amb_out": 50.0}
    return SupermarketFMUEnv(
        fmu_path=cfg.fmu_path,
        horizon=cfg.horizon,
        sim_dt=cfg.sim_dt,
        ctrl_dt=cfg.ctrl_dt,
        Psuc_ref_bounds=cfg.Psuc_ref_bounds,
        Troom_ref_bounds_C=cfg.Troom_ref_bounds_C,
        Psuc_ref_init=cfg.Psuc_ref_init,
        Troom_ref_init_C=cfg.Troom_ref_init_C,
        electricity_price_eur_per_kwh=10.0,
        fan_power_coeff=cfg.fan_power_coeff,
        price_reward_weight=cfg.price_reward_weight,
        T_wmean_min_C=cfg.T_wmean_min_C,
        T_wmean_max_C=cfg.T_wmean_max_C,
        temp_window_seconds=cfg.temp_window_seconds,
        temp_weighted_plateau_hours=cfg.temp_weighted_plateau_hours,
        temp_weighted_decay_hours=cfg.temp_weighted_decay_hours,
        temp_weighted_floor=cfg.temp_weighted_floor,
        include_weighted_temp_in_obs=cfg.include_weighted_temp_in_obs,
        T_weighted_div=cfg.T_weighted_div,
        terminate_on_window_mean_violation=cfg.terminate_on_window_mean_violation,
        termination_buffer_seconds=cfg.termination_buffer_seconds,
        alive_reward_per_ctrl_step=cfg.alive_reward_per_ctrl_step,
        action_lpf_alpha=cfg.action_lpf_alpha,
        action_smoothness_coef=cfg.action_smoothness_coef,
        action_smoothness_type=cfg.action_smoothness_type,
        action_smoothness_use_scaled=cfg.action_smoothness_use_scaled,
        include_window_fill_in_obs=cfg.include_window_fill_in_obs,
        include_applied_action_in_obs=cfg.include_applied_action_in_obs,
        use_excel_profile=cfg.use_excel_profile,
        excel_path=cfg.excel_path,
        episode_hours=cfg.episode_hours,
        forecast_hours=cfg.forecast_hours,
        price_is_eur_per_mwh=cfg.price_is_eur_per_mwh,
        scale_observations=cfg.scale_observations,
        scale_time_feature=cfg.scale_time_feature,
        scale_forecast=cfg.scale_forecast,
        forecast_price_div=cfg.forecast_price_div,
        obs_divisors=obs_divisors,
        T_wmean_div=1.0,
        violation_clip_C=cfg.violation_clip_C,
        terminate_on_obs_nan=True,
        debug_print_on_fail=False,
        max_reset_retries=3,
    )
...
def main():
    cfg = Config()
    cfg.frames_per_batch = int(cfg.chunk_T * cfg.num_envs)
    os.makedirs(cfg.save_dir, exist_ok=True)
    run_dir = os.path.join(cfg.save_dir, cfg.run_name)
    os.makedirs(run_dir, exist_ok=True)
    batch_metrics_csv = os.path.join(run_dir, 'batch_metrics_publicpred.csv')

    set_seed(cfg.seed)

    is_fork = mp.get_start_method(allow_none=True) == "fork"
    device = torch.device(cfg.device if (torch.cuda.is_available() and not is_fork) else "cpu")
    ...
    INFO_KEYS = [
        "term_code", "Tcabinet_C", "T_wmean_C", "T_weighted_C", "temp_window_filled",
        "T_wmean_violation_always", "T_wmean_violation_full", "T_wmean_violation",
        "T_weighted_violation", "T_weighted_violation_always", "T_weighted_violated_always",
        "step_cost_eur", "episode_end", "episode_len_steps", "episode_len_seconds",
        "step_cost_eur_raw", "step_cost_eur_total_raw", "step_cost_eur_used",
        "price_reward_weight", "alive_reward_per_ctrl_step", "smooth_pen", "smooth_coef",
        "c_elec_eur_per_kwh", "c_elec_used_eur_per_kwh", "reward_total", "truncation_penalty",
        "Twmean_min_C", "Twmean_max_C", "Twmean_violation_clip_C"
    ]

    LOW = torch.tensor([cfg.Psuc_ref_bounds[0], cfg.Troom_ref_bounds_C[0]], device=device)
    HIGH = torch.tensor([cfg.Psuc_ref_bounds[1], cfg.Troom_ref_bounds_C[1]], device=device)
    ...
    env = ParallelEnv(cfg.num_envs, make_env, mp_start_method="spawn")
    check_env_specs(env)

    obs_dim = int(env.observation_spec["observation"].shape[-1])
    act_dim = int(env.action_spec.shape[-1])
    if getattr(cfg, 'forecast_ambient_obs_indices', None) is None and int(getattr(cfg, 'forecast_hours', 0)) > 0 and obs_dim >= int(getattr(cfg, 'forecast_hours', 0)):
        cfg.forecast_ambient_obs_indices = tuple(range(obs_dim - int(cfg.forecast_hours), obs_dim))
    if getattr(cfg, 'current_temperature_obs_index', None) is None:
        cfg.current_temperature_obs_index = _infer_default_current_temperature_obs_index(obs_dim, cfg)
    print(f"obs_dim: {obs_dim} | act_dim: {act_dim}")
    print(f"dynamics_exog_obs_indices: {getattr(cfg, 'dynamics_exog_obs_indices', None)} | forecast_ambient_obs_indices: {getattr(cfg, 'forecast_ambient_obs_indices', None)} | current_temperature_obs_index: {getattr(cfg, 'current_temperature_obs_index', None)}")

    actor_feature = make_feature_module("observation", "actor_embed", cfg.actor_num_cells, device)
    ...
    exog_probe_dim = len(_sanitize_index_list(getattr(cfg, 'dynamics_exog_obs_indices', None), obs_dim))

    state_proj_core = LatentStateProjector(
        cfg.actor_lstm_hidden,
        cfg.dyn_state_dim,
        cfg.dyn_hidden,
        obs_clip=float(getattr(cfg, 'state_projector_obs_clip', 10.0)),
    ).to(device)
    actor_state_proj = TensorDictModule(
        state_proj_core,
        in_keys=["actor_xfm_latent", "observation"],
    ...
    clamp_loc_scale = TensorDictModule(ClampLocScale(1e-3, 10.0), in_keys=["loc", "scale"], out_keys=["loc", "scale"]).to(device)
    ...
    reward_critic = CriticMLP(obs_dim).to(device)
    cost_critic = CriticMLP(obs_dim).to(device)
    ...
    dynamics_params = unique_params(actor_feature, actor_lstm, actor_rollbuf, actor_xfm, state_proj_core, dyn_model, temp_decoder)
    # Match the Sinergym HVAC script: PPO is allowed to update the full policy path,
    # including the shared latent projector and switching dynamics used to condition the actor.
    # The dynamics optimizer then applies the supervised dynamics/temperature update as a second,
    # separate step on the shared representation, just like the HVAC code.
    ...
    with torch.no_grad():
        ...
        probe_exog = extract_dynamics_exog_tensor(td0_probe["observation"].float(), cfg)
        _ = dyn_model.regime_probs(td0_probe["actor_dyn_state"].float(), probe_exog)
        _ = temp_decoder(td0_probe["actor_dyn_state"].float(), probe_exog)
    ...
    def prepare_recurrent_minibatch(sub_td, *, time_dim: int):
        return strip_recurrent_to_t0(
            sub_td,
            ["actor_h", "actor_c", "actor_window"],
            time_dim=time_dim,
        )
    ...
    def eval_new_logp_and_entropy(sub_td):
        ...
        dist = TanhNormal(loc, torch.clamp(scale, 1e-3, 1.0), low=LOW, high=HIGH)
        ...
        ent = (-logp).mean()
        require_finite(ent, "entropy proxy")
        return logp, ent
    ...
    pbar = tqdm(total=cfg.total_frames, bar_format="{desc}")
    frames = 0
    last_it = -1
    ...
    nan_check_keys = [
        ("next", "observation"), ("next", "reward"), ("next", "step_cost_eur"),
        ("next", "Tcabinet_C"), ("next", "T_wmean_C"), ("next", "T_weighted_C"),
        ("next", "T_wmean_violation_always"), ("next", "T_wmean_violation_full"),
        ("next", "T_weighted_violation_always"), ("next", "Twmean_min_C"),
        ("next", "Twmean_max_C"), ("action",), ("action_log_prob",), ("is_init",)
    ]
    ...
    try:
        for it, td in enumerate(collector):
            ...
            with torch.no_grad():
                ...
                next_obs = td["next", "observation"].float()
                td["state_value"] = reward_critic(obs)
                td["next", "state_value"] = reward_critic(next_obs)
                td["cost_value"] = cost_critic(obs)
                td["next", "cost_value"] = cost_critic(next_obs)
                td["next", "constraint"] = make_constraint_from_info(td, cfg)
            ...
            td["advantage"] = td["advantage_r"] - lag_lambda.detach() * td["advantage_c"]
            normalize_advantage_(td, adv_key="advantage", eps=cfg.eps)

            with torch.no_grad():
                batch_cost_mean, _, _, _ = batch_constraint_stats(td)
                if it >= cfg.lambda_warmup_batches:
                    lag_lambda = torch.clamp(
                        lag_lambda + cfg.lambda_lr * (batch_cost_mean - cfg.cost_budget),
                        0.0,
                        cfg.lambda_max,
                    )
            ...
            for _epoch in range(cfg.ppo_epochs):
                for sub, _start in contiguous_minibatches(td, cfg.chunk_T, time_dim=time_dim):
                    ...
                    # Use two fresh minibatch views. Re-running actor_trunk on the exact same
                    # TensorDict after a backward is unsafe here because actor_trunk carries
                    # recurrent state forward in-place (actor_h/actor_c/actor_window). After the
                    # first pass those fields point to tensors attached to the first autograd
                    # graph, so a second forward/backward can accidentally traverse the old graph
                    # again and trigger "backward through the graph a second time".
                    sub_actor = sub.clone()
                    sub_dyn = sub.clone()
                    ...
                    with set_recurrent_mode(True):
                        try:
                            ...
                            loss_pi_raw, loss_ent_raw, loss_ent_scaled, _, pi_logs = ppo_actor_loss_with_logs(sub_actor)

                            loss_vr_raw = F.smooth_l1_loss(pred_r, sub_actor["value_target_r"].detach().squeeze(-1))
                            loss_vr_total = cfg.reward_value_coef * loss_vr_raw

                            loss_vc_raw = F.smooth_l1_loss(pred_c, sub_actor["value_target_c"].detach().squeeze(-1))
                            loss_vc_total = cfg.cost_value_coef * loss_vc_raw

                            actor_total = loss_pi_raw + loss_ent_scaled
                        ...
                        try:
                            loss_dyn_raw, dyn_logs = latent_dynamics_loss(
                                sub_dyn,
                                state_proj_core,
                                dyn_model,
                                temp_decoder,
                        ...
                        mb_count += 1
                        losses["actor_loss_raw"] += float(loss_pi_raw.detach().cpu())
                        losses["actor_loss_total"] += float(actor_total.detach().cpu())
                        losses["reward_value_loss_raw"] += float(loss_vr_raw.detach().cpu())
                        losses["reward_value_loss_total"] += float(loss_vr_total.detach().cpu())
                        losses["cost_value_loss_raw"] += float(loss_vc_raw.detach().cpu())
                        losses["cost_value_loss_total"] += float(loss_vc_total.detach().cpu())
                        losses["loss_ent_raw"] += float(loss_ent_raw.detach().cpu())
                        losses["loss_ent_scaled"] += float(loss_ent_scaled.detach().cpu())
                        losses["loss_dyn_raw"] += float(loss_dyn_raw.detach().cpu())
                        ...
                        if torch.cuda.is_available() and int(getattr(cfg, "cuda_empty_cache_every_minibatches", 0)) > 0 and (mb_count % int(getattr(cfg, "cuda_empty_cache_every_minibatches", 0)) == 0):
                            torch.cuda.empty_cache()

            if torch.cuda.is_available() and bool(getattr(cfg, "empty_cache_after_checkpoint", True)):
                torch.cuda.empty_cache()

            with torch.no_grad():
                ev_r = explained_variance(reward_critic(td["observation"].float()), td["value_target_r"].squeeze(-1))
                ev_c = explained_variance(cost_critic(td["observation"].float()), td["value_target_c"].squeeze(-1))

            if it % cfg.log_every_batches == 0:
                denom = max(1, mb_count)
                c_tb = to_TB(td["next", "constraint"].float(), time_dim=time_dim)
                if c_tb.ndim > 2:
                    c_tb = c_tb.squeeze(-1)
                ...
                lines = [
                    ...
                    f"done={mean_done_rate(td, time_dim=time_dim):.2%} | lambda={float(lag_lambda):.4f} | "
                    f"constraint μ={c_tb.mean().item():.6f} σ={c_tb.std(unbiased=False).item():.6f} "
                    f"max={c_tb.max().item():.6f} | active={(c_tb > 1e-8).float().mean().item():.2%}",
                    f"episodes: ended={len(ended_lengths)} | "
                    f"ended_len_mean={(float(np.mean(ended_lengths)) if ended_lengths else float('nan')):.2f} | "
                    f"running_len_mean={running_mean_len:.2f}",
                ]

                if len(term_counts) > 0:
                    lines.append("term_codes: " + ", ".join([f"{k}:{v}" for k, v in term_counts.most_common(6)]))

                rmean = mean_of(td, "reward", time_dim=time_dim)
                sc_mean = mean_of(td, "step_cost_eur", time_dim=time_dim)
                if rmean is not None and sc_mean is not None:
                    lines.append(f"reward μ={rmean:.6f} | step_cost μ={sc_mean:.6f}")

                if safe_key(td, ("action",)) is not None:
                    lines.append(action_stats_by_dim(safe_key(td, ("action",)), name="action_sample"))

                Tc = mean_std_max(td, "Tcabinet_C", time_dim=time_dim)
                Tw = mean_std_max(td, "T_wmean_C", time_dim=time_dim)
                Twg = mean_std_max(td, "T_weighted_C", time_dim=time_dim)
                if Tc:
                    lines.append(f"Tcab: μ={Tc[0]:.4f} σ={Tc[1]:.4f} max={Tc[2]:.4f}")
                if Tw:
                    lines.append(f"Twmean: μ={Tw[0]:.4f} σ={Tw[1]:.4f} max={Tw[2]:.4f} min={Tw[3]:.4f}")
                if Twg:
                    lines.append(f"Tweighted: μ={Twg[0]:.4f} σ={Twg[1]:.4f} max={Twg[2]:.4f}")

                v_weighted_td = safe_key(td, ("next", "T_weighted_violation_always"))
                if v_weighted_td is not None:
                    lines.append(rollout_summary_line(to_TB(v_weighted_td, time_dim).float(), "weighted_violation"))

                lines.append(
                    f"PPO: KL={losses['approx_kl']/denom:.6f} | clipfrac={losses['clipfrac']/denom:.4f} | "
                    f"ratio μ={losses['ratio_mean']/denom:.4f} σ={losses['ratio_std']/denom:.4f} | "
                    f"entropy={losses['entropy']/denom:.4f}"
                )
                lines.append(
                    f"losses: actor={losses['actor_loss_total']/denom:.4f} | "
                    f"Vr={losses['reward_value_loss_total']/denom:.4f} | "
                    f"Vc={losses['cost_value_loss_total']/denom:.4f} | "
                    f"dyn_total={losses['dynamics_total']/denom:.6f} | "
                    f"dyn={losses['loss_dyn_scaled']/denom:.6f}"
                )
                lines.append(
                    f"latent_dyn: enabled={(losses['latent_dynamics_loss_enabled']/denom) >= 0.5} | "
                    f"one_step={losses['dyn_1step']/denom:.6f} | "
                    f"one_step_rmse={losses['dyn_1step_rmse']/denom:.6f} | "
                    f"one_step_nrmse={losses['dyn_1step_nrmse']/denom:.6f} | "
                    f"roll_rmse={losses['dyn_roll_rmse']/denom:.6f} | "
                    f"roll_nrmse={losses['dyn_roll_nrmse']/denom:.6f} | "
                    f"regime_maxprob={losses['regime_maxprob']/denom:.4f}"
                )
                lines.append(
                    f"temp/viol supervision: temp_dyn1={losses['loss_temp_res_dyn1']/denom:.6f} | "
                    f"temp_roll={losses['loss_temp_res_roll']/denom:.6f} | "
                    f"viol_dyn1={losses['loss_viol_dyn1']/denom:.6f} | "
                    f"viol_roll={losses['loss_viol_roll']/denom:.6f}"
                )
                lines.append(
                    f"vs persistence penalties: temp_dyn1={losses['loss_temp_persist_dyn1']/denom:.6f} | "
                    f"temp_roll={losses['loss_temp_persist_roll']/denom:.6f} | "
                    f"viol_dyn1={losses['loss_viol_persist_dyn1']/denom:.6f} | "
                    f"viol_roll={losses['loss_viol_persist_roll']/denom:.6f}"
                )
                lines.append(
                    f"gain vs persist (nrmse): temp_last={losses['temp_gain_nrmse_last_h']/denom:.4f} | "
                    f"temp_min={losses['temp_gain_nrmse_min_h']/denom:.4f} | "
                    f"viol_last={losses['viol_gain_nrmse_last_h']/denom:.4f} | "
                    f"viol_min={losses['viol_gain_nrmse_min_h']/denom:.4f}"
                )
                lines.append(
                    f"temperature decoder: pred μ={losses['decoded_temperature_mean']/denom:.4f} σ={losses['decoded_temperature_std']/denom:.4f} "
                    f"min={losses['decoded_temperature_min']/denom:.4f} max={losses['decoded_temperature_max']/denom:.4f} | "
                    f"latent_norm={losses['latent_state_norm']/denom:.4f}"
                )
                lines.append(
                    f"temp fit (open-loop monitor): direct_rmse={losses['latent_temperature_direct_rmse']/denom:.4f} | "
                    f"dyn1_rmse={losses['latent_temperature_dyn1_rmse']/denom:.4f} | "
                    f"roll_rmse={losses['latent_temperature_roll_rmse']/denom:.4f} | "
                    f"direct_nrmse={losses['latent_temperature_direct_nrmse']/denom:.4f} | "
                    f"dyn1_nrmse={losses['latent_temperature_dyn1_nrmse']/denom:.4f} | "
                    f"roll_nrmse={losses['latent_temperature_roll_nrmse']/denom:.4f}"
                )
                lines.append(
                    f"temp persistence baseline: dyn1_rmse={losses['temp_persistence_dyn1_rmse']/denom:.4f} | "
                    f"roll_rmse={losses['temp_persistence_roll_rmse']/denom:.4f} | "
                    f"dyn1_nrmse={losses['temp_persistence_dyn1_nrmse']/denom:.4f} | "
                    f"roll_nrmse={losses['temp_persistence_roll_nrmse']/denom:.4f} | "
                    f"gain_dyn1={losses['temp_rmse_gain_dyn1']/denom:.4f} | "
                    f"gain_roll={losses['temp_rmse_gain_roll']/denom:.4f}"
                )
                lines.append(
                    f"violation fit (open-loop): dyn1_rmse={losses['viol_dyn1_rmse']/denom:.4f} | "
                    f"roll_rmse={losses['viol_roll_rmse']/denom:.4f} | "
                    f"dyn1_nrmse={losses['viol_dyn1_nrmse']/denom:.4f} | "
                    f"roll_nrmse={losses['viol_roll_nrmse']/denom:.4f} | "
                    f"persist_roll_rmse={losses['viol_persistence_roll_rmse']/denom:.4f} | "
                    f"gain_roll={losses['viol_rmse_gain_roll']/denom:.4f}"
                )
                ...
                if shield_active_td is not None:
                    ...
                    lines.append(
                        f"shield: eval_on={(se.mean().item() if se is not None else 0.0):.2%} | apply_on={(sp.mean().item() if sp is not None else float(current_shield_apply)):.2%} | "
                        f"apply_after_batch={int(cfg.shield_apply_after_batch)} | active_rate={sa.mean().item():.2%} | "
                        f"changed_rate={(sc.mean().item() if sc is not None else 0.0):.2%} | would_change_rate={(sw.mean().item() if sw is not None else 0.0):.2%} | "
                        f"base_pred_v={(sb.mean().item() if sb is not None else 0.0):.4f} | best_pred_v={(sx.mean().item() if sx is not None else 0.0):.4f} | final_pred_v={(sf.mean().item() if sf is not None else 0.0):.4f} | "
                        f"delta_norm={(sd.mean().item() if sd is not None else 0.0):.4f} | would_delta_norm={(swd.mean().item() if swd is not None else 0.0):.4f}"
                    )
                horizons = choose_logging_horizons(int(cfg.dyn_roll_horizon))
                lines.append(
                    f"beats persistence?: temp_dyn1={('yes' if (losses['temp_beats_persist_dyn1']/denom) >= 0.5 else 'no')} | "
                    f"temp_roll={('yes' if (losses['temp_beats_persist_roll']/denom) >= 0.5 else 'no')} | "
                    f"viol_dyn1={('yes' if (losses['viol_beats_persist_dyn1']/denom) >= 0.5 else 'no')} | "
                    f"viol_roll={('yes' if (losses['viol_beats_persist_roll']/denom) >= 0.5 else 'no')}"
                )
                lines.append(
                    f"surplus vs persist (negative is better): temp_dyn1={losses['temp_vs_persist_dyn1_surplus']/denom:+.4f} | "
                    f"temp_roll={losses['temp_vs_persist_roll_surplus']/denom:+.4f} | "
                    f"viol_dyn1={losses['viol_vs_persist_dyn1_surplus']/denom:+.4f} | "
                    f"viol_roll={losses['viol_vs_persist_roll_surplus']/denom:+.4f}"
                )
                lines.append(format_horizon_triplets(
                    losses, denom,
                    model_prefix='latent_temperature_nrmse',
                    persist_prefix='latent_temperature_persist_nrmse',
                    gain_prefix='latent_temperature_gain_nrmse',
                    horizons=horizons,
                    label='temp horizon nrmse / persist / gain',
                ))
                lines.append(format_horizon_triplets(
                    losses, denom,
                    model_prefix='latent_violation_nrmse',
                    persist_prefix='latent_violation_persist_nrmse',
                    gain_prefix='latent_violation_gain_nrmse',
                    horizons=horizons,
                    label='viol horizon nrmse / persist / gain',
                ))
                lines.append(
                    f"decoder bias/corr: direct_bias={losses['latent_temperature_direct_bias']/denom:+.4f} | "
                    f"dyn1_bias={losses['latent_temperature_dyn1_bias']/denom:+.4f} | "
                    f"roll_bias={losses['latent_temperature_roll_bias']/denom:+.4f} | "
                    f"direct_corr={losses['latent_temperature_direct_corr']/denom:+.4f} | "
                    f"dyn1_corr={losses['latent_temperature_dyn1_corr']/denom:+.4f}"
                )
                ...
                batch_row = {
                    'batch': int(it),
                    'frames': int(frames),
                    'lambda': float(lag_lambda),
                    'done_rate': float(mean_done_rate(td, time_dim=time_dim)),
                    'constraint_mean': float(c_tb.mean().item()),
                    'constraint_std': float(c_tb.std(unbiased=False).item()),
                    'constraint_max': float(c_tb.max().item()),
                    'constraint_active_rate': float((c_tb > 1e-8).float().mean().item()),
                    'reward_mean': float(rmean) if rmean is not None else float('nan'),
                    'step_cost_mean': float(sc_mean) if sc_mean is not None else float('nan'),
                    'explained_variance_reward': float(ev_r),
                    'explained_variance_cost': float(ev_c),
                    'dyn_rollout_progress': float(losses['dyn_rollout_progress']/denom),
                    'latent_dynamics_loss_enabled': float(losses['latent_dynamics_loss_enabled']/denom),
                    'latent_dyn_one_step_nrmse': float(losses['dyn_1step_nrmse']/denom),
                    'latent_dyn_roll_nrmse': float(losses['dyn_roll_nrmse']/denom),
                    'temp_direct_rmse': float(losses['latent_temperature_direct_rmse']/denom),
                    'temp_dyn1_rmse': float(losses['latent_temperature_dyn1_rmse']/denom),
                    'temp_roll_rmse': float(losses['latent_temperature_roll_rmse']/denom),
                    'temp_dyn1_nrmse': float(losses['latent_temperature_dyn1_nrmse']/denom),
                    'temp_roll_nrmse': float(losses['latent_temperature_roll_nrmse']/denom),
                    'temp_persist_dyn1_nrmse': float(losses['temp_persistence_dyn1_nrmse']/denom),
                    'temp_persist_roll_nrmse': float(losses['temp_persistence_roll_nrmse']/denom),
                    'temp_gain_nrmse_last_h': float(losses['temp_gain_nrmse_last_h']/denom),
                    'temp_gain_nrmse_min_h': float(losses['temp_gain_nrmse_min_h']/denom),
                    'temp_beats_persist_dyn1': float(losses['temp_beats_persist_dyn1']/denom),
                    'temp_beats_persist_roll': float(losses['temp_beats_persist_roll']/denom),
                    'temp_vs_persist_dyn1_surplus': float(losses['temp_vs_persist_dyn1_surplus']/denom),
                    'temp_vs_persist_roll_surplus': float(losses['temp_vs_persist_roll_surplus']/denom),
                    'temp_direct_bias': float(losses['latent_temperature_direct_bias']/denom),
                    'temp_dyn1_bias': float(losses['latent_temperature_dyn1_bias']/denom),
                    'temp_roll_bias': float(losses['latent_temperature_roll_bias']/denom),
                    'temp_direct_corr': float(losses['latent_temperature_direct_corr']/denom),
                    'temp_dyn1_corr': float(losses['latent_temperature_dyn1_corr']/denom),
                    'viol_dyn1_rmse': float(losses['viol_dyn1_rmse']/denom),
                    'viol_roll_rmse': float(losses['viol_roll_rmse']/denom),
                    'viol_dyn1_nrmse': float(losses['viol_dyn1_nrmse']/denom),
                    'viol_roll_nrmse': float(losses['viol_roll_nrmse']/denom),
                    'viol_persist_dyn1_nrmse': float(losses['viol_persistence_dyn1_nrmse']/denom),
                    'viol_persist_roll_nrmse': float(losses['viol_persistence_roll_nrmse']/denom),
                    'viol_gain_nrmse_last_h': float(losses['viol_gain_nrmse_last_h']/denom),
                    'viol_gain_nrmse_min_h': float(losses['viol_gain_nrmse_min_h']/denom),
                    'viol_beats_persist_dyn1': float(losses['viol_beats_persist_dyn1']/denom),
                    'viol_beats_persist_roll': float(losses['viol_beats_persist_roll']/denom),
                    'viol_vs_persist_dyn1_surplus': float(losses['viol_vs_persist_dyn1_surplus']/denom),
                    'viol_vs_persist_roll_surplus': float(losses['viol_vs_persist_roll_surplus']/denom),
                }
                for h in horizons:
                    batch_row[f'temp_nrmse_h{h:02d}'] = float(losses[f'latent_temperature_nrmse_h{h:02d}']/denom)
                    batch_row[f'temp_persist_nrmse_h{h:02d}'] = float(losses[f'latent_temperature_persist_nrmse_h{h:02d}']/denom)
                    batch_row[f'temp_gain_nrmse_h{h:02d}'] = float(losses[f'latent_temperature_gain_nrmse_h{h:02d}']/denom)
                    batch_row[f'viol_nrmse_h{h:02d}'] = float(losses[f'latent_violation_nrmse_h{h:02d}']/denom)
                    batch_row[f'viol_persist_nrmse_h{h:02d}'] = float(losses[f'latent_violation_persist_nrmse_h{h:02d}']/denom)
                    batch_row[f'viol_gain_nrmse_h{h:02d}'] = float(losses[f'latent_violation_gain_nrmse_h{h:02d}']/denom)
                append_batch_metrics_csv(batch_metrics_csv, batch_row)
                pbar.write("\n".join(lines))
